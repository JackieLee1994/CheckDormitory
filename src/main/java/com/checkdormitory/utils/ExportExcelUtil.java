...
public class ExportExcelUtil {
	...
	public  Workbook writeNewExcel(File file,String sheetName,List<StudentInfo> lis) throws Exception{
		Workbook wb = null;
		Row row = null; 
		Cell cell = null;
		...
		for (int i = 0; i < lis.size(); i++) {
			...
			StudentInfo studentInfo = lis.get(i);
			if(null==studentInfo){
				break;
			}
			//Cell赋值开始
			cell = row.createCell(0);
			cell.setCellValue(studentInfo.getStuNumber());
			...
			cell.setCellValue(studentInfo.getClazz());
			cell.setCellStyle(cs);
			
			cell = row.createCell(3);
			cell.setCellValue(studentInfo.getDormitoryBuilding()+studentInfo.getDormitoryNumber());
			cell.setCellStyle(cs);

			cell = row.createCell(4);
			cell.setCellValue(studentInfo.getNoComingSum());
			cell.setCellStyle(cs);
		}
		return wb;
	}
